...
app.get('api/movie-reviews', (req, res) => {
  db.query(`SELECT movies.movie_name AS Name, reviews.review
  FROM movies INNER JOIN reviews
  ON reviews.movie_id = movies.id`), function (err, results) {
    if (err) {
      console.log(err);
    }
    console.log(results);
...
app.put('api/review/:id', (req , res) => {
  ...
  db.query(`UPDATE reviews SET review = ${updatedReview} WHERE movie_id = ${selectedMovie}`, function (err, results) {
    if (err) {
      console.log(err)
    }
    else {
